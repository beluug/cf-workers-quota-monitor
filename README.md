...
| 设备 | 推荐下载 | 说明 |
|---|---|---|
| 普通Windows电脑（Intel/AMD） | `CF-Quota-Monitor-v1.0.2-Windows-x64-Setup.exe` | Windows 10/11，绝大多数电脑选择这个 |
| Windows ARM电脑（骁龙） | `CF-Quota-Monitor-v1.0.2-Windows-arm64-Setup.exe` | 仅适用于ARM64电脑 |
| 不想安装 | 对应架构的 `Portable.zip` | 解压后运行 `CFQuotaMonitor.exe` |
| Android手机 | `CF-Quota-Monitor-v1.3.1.apk` | Android 8.0及以上 |
...
| 功能 | Android v1.3.1 | Windows v1.0.2 |
|---|:---:|:---:|
| 多账号同屏与进度条 | ✓ | ✓ |
| 打开时立即刷新 | ✓ | ✓ |
| 自选后台刷新频率 | ✓ | ✓（托盘运行） |
...
Android v1.3.1和Windows v1.0.2都可以选择已保存账户并导出为`.cfqm`文件，文件可在两个平台之间互相导入。Windows也可以直接导入旧版Android生成的`.cfqm.json`文件：
...
备份密码无法找回。格式说明见 [CFQM备份格式](docs/CFQM_BACKUP_FORMAT.md)。
...
The Windows v1.0.2 artifacts are currently unsigned. Future approved Windows releases will use a verifiable SignPath build and manual signing approval process.
